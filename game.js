...
`鏡靈會反射勇者的情緒，讓遠征隊的恐懼感逐漸加深，最後發狂逃跑，因此，遠征隊嘗試使用安定咒語來克服恐懼。若恐懼下降，才能繼續勇往直前，毛丸的牢籠也會更接近。`,
    // ✅ 學術任務更新（25人）
    story2:
`學術任務：遠征隊勇者25人，比較他們在使用安定咒語前後的恐懼值是否顯著下降，回報 z 與 Wilcoxon 的 W。`,
...

  {
    ...
    img: "image/wolves.png",
     // ✅ 提示更新
    hint: "比較兩條突圍路線，看哪一條穿越的時間最短。",
    mission:
`眼前出現兩條岔路，遠征隊要兵分兩路前進。
為了爭取時間，必須找出穿越速度最快的路線。`,
    // ✅ 路徑命名更新：路徑A:濕冷岩道、路徑B:地下河流
    story1:
  `裂谷像把王國剖開，遠征隊兵分兩路：路線A「濕冷岩道」與路線B「地下河流」。你手上的數據是每位隊員穿越路徑所花的時間，越短越能更快接近毛丸。`,
    // ✅ 學術任務更新
    story2:
`學術任務：比較兩條路線的行進時間是否有顯著差異，以及哪條路徑需要的時間最短，回報 U 與 p。`,

    baseInputs: [
      { key:"u", label:"U 值", answer:446, tol:1.1 },
      { key:"p", label:"p 值", answer:0.010, tol:TOL_P }
    ],
    extraInput: { key:"meanPathB", label:"加強挑戰：路線B（地下河流）平均時間", answer:67.66, tol:0.05 }
  },
